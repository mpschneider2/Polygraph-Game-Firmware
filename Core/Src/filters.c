...
void filter_init_FIR(FIR_filter_t * filter) {
	memset(filter->raw_data,sizeof(filter->raw_data));
	memset(filter->filtered_data,sizeof(filter->filtered_data));
	filter->idx = 0;
	filter->sum = 0;
}
...
void filter_update_FIR(FIR_filter_t * filter, uint16_t val) {
	...
	filter->raw_data[idx] = val;
	filter->sum += val;
	filter->filtered_data[idx] = val/sum;
